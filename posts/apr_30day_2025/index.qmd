...
plot_container[[as.character(i)]] <-
  ...
  mutate(
    ...
    scale_x_continuous(expand = c(0,0), breaks = seq(0, 48, length.out = 12), labels = c("Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"))+
    theme(
        legend.position = "none",
        panel.grid = element_blank(),
        axis.ticks = element_blank(),
...
library(ggmap)        # if you want static basemaps
library(osmdata)      # to get your municipality borders
library(geosphere)    # for distances
library(XML)          # to parse GPX

# We need to load all the activity files from the Strava data export
# We load all the .gpx files

gpx_files <- list.files("../../strava_export/activities", pattern = "\\.gpx$", full.names = TRUE)
length(gpx_files) #727 files
# It seems Strava also saved many activities as .fit files
library(FITfileR); library(R.utils)
read_fit <- function(file) {
  fit_data <- readFitFile(file)
  # Extract relevant data from fit_data as needed
  # This will depend on the structure of your FIT files
  # For example:
  records <- records(fit_data)
  # Convert records to a tibble or data frame
  # ...
  return(records)
}
fit_gz_files <- list.files("../../strava_export/activities", pattern = "\\.fit.gz$", full.names = TRUE)
# Decompress the gz 
# Function to decompress and read a .fit file
read_fit_gz <- function(gz_file) {
  # Decompress the .gz file (without removing the original)
  fit_file <- gunzip(gz_file, remove = FALSE, overwrite = TRUE)
  
  # Read the .fit file
  fit_data <- readFitFile(fit_file)
  
  return(fit_data)
}

# Apply the function to all .fit.gz files
fit_data_list <- lapply(fit_gz_files, read_fit_gz)

# Test with a few
#gpx_files <- sample(gpx_files, 50)

# We define a function to extract GPS data 
read_gpx <- function(file) {
  trkpts <- xmlParse(file) %>%
    xmlToList() %>%
    purrr::pluck("trk", "trkseg") %>%
    map_dfr(~ tibble(
      lat = as.numeric(.x$.attrs["lat"]),
      lon = as.numeric(.x$.attrs["lon"]),
      ele = as.numeric(.x$ele),
      time = as.POSIXct(.x$time, format = "%Y-%m-%dT%H:%M:%SZ", tz = "UTC")
    ))
  trkpts$source_file <- basename(file)
  return(trkpts)
}

# Load all GPX tracks (takes a while)
# all_tracks <- map_dfr(gpx_files, read_gpx) #727 unique source files
#save(all_tracks, file = "all_tracks.RData")
load(file = "all_tracks.RData")

# Filter all_tracks based on whether they can be found in the meta data
# We first collect the file names for all runs in the meta data
metadata_filenames <- strava %>%
               mutate(filename = gsub("activities/", "", filename),
                      filename = gsub(".tcx.gz", "", filename),
                      filename = gsub(".fit.gz", "", filename),
                      filename = gsub(".gpx", "", filename)) %>%
               pull(filename)



# Then we filter the gpx data file for these file names
all_tracks_runs <- 
all_tracks %>% # Has 727 unique runs, but for some reason I cant match all of them (see Isi-run)
    mutate(source_file = gsub(".gpx", "", source_file)) %>%
    filter(source_file %in% metadata_filenames)
```


```{r fig.width = 12, fig.height = 4, include = FALSE, eval = FALSE}
# We only want to look at a heatmap of Bærum, where I live
baerum <- opq("Baerum, Norway") # Collect coordinates

# Convert into sf object
tracks_sf <- all_tracks_runs %>%
  drop_na(lat, lon) %>%
  st_as_sf(coords = c("lon", "lat"), crs = 4326)

tracks_lines <- tracks_sf %>%
  group_by(source_file) %>%
  summarise(do_union = FALSE) %>%  # important for multiple disjoint paths
  st_cast("LINESTRING")

p <-
ggplot(tracks_lines) +
  geom_sf(color = "#6a8782", alpha = 0.1, linewidth = 1) +
    ylim(59.90, 60.05)+xlim(10.3480241, 11.7580825)+
    coord_sf() +
    theme_void()

ggsave("test.jpg", p, width = 600, height = 600, unit = "mm", dpi = 500)
